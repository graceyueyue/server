...
class User_table_tabular: public User_table
{
 public:
  ...
  {
    Field *f= get_field(end_priv_columns);
    return (SSL_type)(f ? f->val_int()-1 : 0);
  }
  int set_ssl_type (SSL_type x) const
  {
    if (Field *f= get_field(end_priv_columns))
  ...
  {
    Field *f= get_field(end_priv_columns + 1);
    return f ? ::get_field(root,f) : 0;
  }
  int set_ssl_cipher (const char *s, size_t l) const
  {
    if (Field *f= get_field(end_priv_columns + 1))
  ...
  {
    Field *f= get_field(end_priv_columns + 2);
    return f ? ::get_field(root,f) : 0;
  }
  int set_x509_issuer (const char *s, size_t l) const
  {
    if (Field *f= get_field(end_priv_columns + 2))
      return f->store(s, l, &my_charset_latin1);
    else
      return 1;
  }
  ...
  {
    Field *f= get_field(end_priv_columns + 3);
    return f ? ::get_field(root,f) : 0;
  }
  int set_x509_subject (const char *s, size_t l) const
  {
    if (Field *f= get_field(end_priv_columns + 3))
      return f->store(s, l, &my_charset_latin1);
    else
      return 1;
  }
  ...
  {
    Field *f= get_field(end_priv_columns + 4);
    return f ? f->val_int() : 0;
  }
  int set_max_questions (longlong x) const
  {
    if (Field *f= get_field(end_priv_columns + 4))
      return f->store(x, 0);
    else
      return 1;
  }
  ...
  {
    Field *f= get_field(end_priv_columns + 5);
    return f ? f->val_int() : 0;
  }
  int set_max_updates (longlong x) const
  {
    if (Field *f= get_field(end_priv_columns + 5))
      return f->store(x, 0);
    else
      return 1;
  }
  ...
  {
    Field *f= get_field(end_priv_columns + 6);
    return f ? f->val_int() : 0;
  }
  int set_max_connections (longlong x) const
  {
    if (Field *f= get_field(end_priv_columns + 6))
      return f->store(x, 0);
    else
      return 1;
  }
  ...
  {
    Field *f= get_field(end_priv_columns + 7);
    return f ? f->val_int() : 0;
  }
  int set_max_user_connections (longlong x) const
  {
    if (Field *f= get_field(end_priv_columns + 7))
      return f->store(x, 0);
    else
      return 1;
  }
  ...
  {
    Field *f= get_field(end_priv_columns + 13);
    return f ? f->val_real() : 0;
  }
  int set_max_statement_time (double x) const
  {
    if (Field *f= get_field(end_priv_columns + 13))
      return f->store(x);
    else
      return 1;
  }
  ...
  {
    Field *f= get_field(end_priv_columns + 11);
    return f ? f->val_int()-1 : 0;
  }
  int set_is_role (bool x) const
  {
    if (Field *f= get_field(end_priv_columns + 11))
      return f->store(x+1, 0);
    else
      return 1;
  }
  ...
  {
    Field *f= get_field(end_priv_columns + 12);
    return f ? ::get_field(root,f) : 0;
  }
  int set_default_role (const char *s, size_t l) const
  {
    if (Field *f= get_field(end_priv_columns + 12))
  ...
  */
  inline Field* get_field(uint field_num) const
  {
    ...
    return m_table->field[field_num];
  }
